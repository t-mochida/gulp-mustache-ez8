...
module.exports = function (view, options, partials) {
    ...
    return through.obj(function (file, enc, cb) {
        ...
        try {
            loadPartials.call(this, template, file.path, options.templateBaseDir);
        } catch (e) {
            this.emit(
                'error',
    ...
    function loadPartials(template, templatePath, templateBaseDir) {
        var templateDir = path.join('assets', 'template');
        if (templateBaseDir) {
            var templateDir = path.resolve(templateBaseDir);
        }
        ...
        while (partialMatch = partialRegexp.exec(template)) {
            ...
            if (!partials[partialName]) {
                try {
                    ...
                    loadPartials.call(this, partial, partialPath, templateBaseDir);
                } catch (ex) {
                     this.emit(
                        'error',
